...
namespace Project2
{
    class Program
    {
        private static IEnumerable<int> userGuesses;
        ...
        static void Main(string[] args)
        {
            ...
            //starting the do-while loop to get the game to restart if the user wants to
            string playAgain;
            do
            {
                ...
                //int[] userGuesses = { userGuessOne, userGuessTwo, userGuessThree, userGuessFour, userGuessFive, userGuessSix };
                {
                    ...
                    userGuesses[0] = userGuessOne;             
                    userGuesses[1] = userGuessTwo;             
                    userGuesses[2] = userGuessThree;          
                    userGuesses[3] = userGuessFour;            
                    userGuesses[4] = userGuessFive;            
                    userGuesses[5] = userGuessSix;             
                ...
                //Intersectional way to guess correct amount
                //var luckyNumbersArray = new[] { 0, 0, 1, 2, 3 };
                //var userGuesses = new[] { 0, 0, 0, 2 };
                //luckyNumbersArray.Intersect(userGuesses);

                int numberGuessedRight = 0;

                foreach (int userGuess in userGuesses)
                {
                    while (userGuess == luckyOne || userGuess == luckyTwo || userGuess == luckyThree || userGuess == luckyFour || userGuess == luckyFive || userGuess == luckySix)
                    {
                        numberGuessedRight++;
                        break;
                    }
                ...
                //Telling the user how many they guessed correctly

                Console.WriteLine("You guessed " + numberGuessedRight + " right!");

                // percentage of numbers guessed correctly == percentage of jackpot user wins

                double percentGuessedCorrectly = numberGuessedRight / 6d;

                double userWinnings = jackpot * percentGuessedCorrectly;

                string userWinningsAmount;
                ...
                if (userWinnings == 0)
                {
                    userWinningsAmount = "$0.00";
                }
                else
                {
                    userWinningsAmount = userWinnings.ToString("C2");
                }
            ...
            while (playAgain == "yes");
            //close out do-while loop from very beginning to make the program run again, or stop
            }
